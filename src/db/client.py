...
class SupabaseClient:
    ...
    def upsert_report(self, report: Report) -> Dict:
        """Insert or update a markdown report for a docket."""
        data = report.to_dict()
        data["updated_at"] = datetime.now(timezone.utc).isoformat()

        result = self.client.table("reports").upsert(
            data, on_conflict="docket_id"
        ).execute()
        return result.data[0] if result.data else {}

    def get_report(self, docket_id: str) -> Optional[Report]:
        """Get the report for a docket."""
        result = (
        ...
        if result.data:
            return Report.from_dict(result.data[0])
        return None

    def get_all_reports(self, limit: int = 50) -> List[Report]:
        ...
        return [Report.from_dict(r) for r in result.data]
